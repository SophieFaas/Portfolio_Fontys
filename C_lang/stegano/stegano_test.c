         ...
         const uint8_t* FirstPixels, const uint8_t* LastPixels)
{
    ...
    fseek(FilePtr,
          ...
          SEEK_SET);
    Result =
                fread(PixelData, sizeof(PixelData[0]), NrPixels * BytesPerPixel,
                      FilePtr);
    TEST_ASSERT_EQUAL(Result, NrPixels * BytesPerPixel);
    TEST_ASSERT_EQUAL_INT8_ARRAY(
                PixelData, FirstPixels, NrPixels * BytesPerPixel);
    ...
    TEST_ASSERT_EQUAL(Result, NrPixels * BytesPerPixel);
    TEST_ASSERT_EQUAL_INT8_ARRAY(
                PixelData, LastPixels, NrPixels * BytesPerPixel);
...
// Krijg bij de laatste twee errors helaas....
